...
ただし、**AI機能（ナラティブAI抽出・AIチャット・音声文字起こし・意味検索）で Gemini API や Claude API を選んだ場合**は、入力内容が各社のサーバーに送信されます。個人情報を含むデータを扱う場合は、Ollama（完全ローカル）を選ぶか、組織の個人情報保護規程と照らし合わせて運用してください。
...
| 更新期限アラート | 音声ファイルの文字起こし |
| 訪問前ブリーフィング | 記録を探す（意味検索） |
...
**症状**: ナラティブAI抽出、AIチャット、音声文字起こし、「記録を探す」が使えない
